...
class MaintenanceMainPage(QWidget):
    def __init__(self, stacked_widget: BaseQStackedWidget):
        ...
        self.unlock_barcode_printer = MainButtonComponent(
            title_html="<h1>Barkod Yazıcı<br/>Kilidini Aç</h1>",
            icon_path='',
            tooltip="Bu buton aracılığı ile barkod yazıcının kilidini açabilirsiniz."
        )

        h_box.addWidget(self.unlock_barcode_printer)

        v_box.addLayout(h_box)

        if footer:
            footer.set_service_and_other_button_width(self.unlock_barcode_printer.width())
    ...
    def __define_events(self):
        self.unlock_barcode_printer.mousePressEvent = lambda event: ConsoleLogger().log("Unlock Barcode Printer")
