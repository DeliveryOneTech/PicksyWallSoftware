...
class RobotManagementMainPage(QWidget):
    def __init__(self, stacked_widget: BaseQStackedWidget, caller_page_number: PageNumber):
        ...
        self.connection_check_button.setStyleSheet(Styles.btn_success(padding="25px 40px"))

        self.robot_check_button = QPushButton('Senaryo Kontrolü')
        self.robot_check_button.setStyleSheet(Styles.btn_success(padding="25px 40px"))

        self.lighting_check_button = QPushButton('Aydınlatma Kontrolü')
        self.lighting_check_button.setStyleSheet(Styles.btn_success(padding="25px 40px"))

        self.relay_check_button = QPushButton('Röle Kontrolü')
        self.relay_check_button.setStyleSheet(Styles.btn_success(padding="25px 40px"))

        self.package_measuring_system_button = QPushButton('Paket Ölçüm Sistemi')
        self.package_measuring_system_button.setStyleSheet(Styles.btn_success(padding="25px 40px"))

        self.restart_button = QPushButton('Cihazı Yeniden Başlat')
        self.restart_button.setStyleSheet(Styles.btn_danger(padding="25px 40px"))
        ...
        grid_layout.addWidget(self.connection_check_button, 0, 0)
        grid_layout.addWidget(self.robot_check_button, 0, 1)
        grid_layout.addWidget(self.lighting_check_button, 0, 2)
        grid_layout.addWidget(self.relay_check_button, 1, 0)
        grid_layout.addWidget(self.package_measuring_system_button, 1, 1)
        grid_layout.addWidget(self.restart_button, 1, 2)
        grid_layout.addWidget(self.x1_axis, 2, 0)
        grid_layout.addWidget(self.z_axis, 2, 1)
        grid_layout.addWidget(self.delivery_cover, 2, 2)
        grid_layout.addWidget(self.x2_axis, 3, 0)
        grid_layout.addWidget(self.y_axis, 3, 1)
        grid_layout.addWidget(self.hide_cover, 3, 2)
